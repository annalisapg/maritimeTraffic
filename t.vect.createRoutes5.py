...
#%flag
#% key: a
#% description: Check if you want to perform an animation (using period mode maps)
#% required: no
#%end
...
def mpr(mapset,d,frame,abbreviation,coordDict,land,outputGates,outputGrid,moments,nfm):
	grass.run_command('g.mapset',flags='c',mapset='tbr'+str(frame))
	grass.run_command('g.copy',vect=land+"@"+mapset+","+land)
	print d
	nomi=[]
	start=[]
	end=[]
	...
	elaborationg frame nr. """+str(frame)
	print """
			
	"""
	all=[]
	calculateMaps(frame,nomi,abbreviation,coordDict,land,start,end,outputGates,outputGrid)
	nfm.writelines(outputGates+'@tbr'+str(frame))
	nfm.writelines('\n')
	nfm.writelines(outputGrid+'@tbr'+str(frame))
	nfm.writelines('\n')
...
def main():
	...
	animation = flags['a']
	#read the current mapset
	mapset=grass.read_command('g.mapset',flags='p').split('\n')[0]
	...
	grass.run_command('t.create',output='traffic',type='stvds',title='request',description='request')
	grass.run_command('t.create',output='trafficG',type='stvds',title='request',description='request')
	if moment:
		momentM = datetime.datetime.strptime(moment, "%Y-%m-%d %H:%M:%S")
		idxs=[]
		n=0
		for i in startI:
			iM = datetime.datetime.strptime(i, "%Y-%m-%d %H:%M:%S")
			endIM = datetime.datetime.strptime(endI[n], "%Y-%m-%d %H:%M:%S")
			if momentM >= iM and momentM <= endIM:
		...
		grass.run_command('t.register',input='traffic',maps=outputGates,type='vect',start=moment,end=moment)
		grass.run_command('t.register',input='traffic',maps=outputGrid,type='vect',start=moment,end=moment)
		grass.run_command('g.remove',vect='gates,gridCat')
	#continuous - I obtain a list of lists, subsets of d
	if period:
		pC=open('passagesChaque.txt','w')
		moments=[]
		st,en=period.split(',')
		...
		#add final situation
		moments.append(en)
		#necessary because the while construct needs to surpass the limit of the period (and writes an out-of-bounds datetime)		
		del moments[-2]
		di=[]
		n=0
		for m in moments[:-1]:
			...
			for i in startI:
				g=g+1
				if (i >= m and i < moments[n+1]) or (endI[g-1] >= m and endI[g-1] < moments[n+1]):
					idxs.append(startI.index(i))
			n=n+1
			tlist=[d[j] for j in idxs]
			di.append(tlist)
		pC.writelines(str(di))
		pC.close()

		print moments
		
		#multiprocesing begins here
		nfm=open('nomiFmaps','w')
		#define an output queue
		outputQ = mp.Queue()
		#set up a list of process to run
		processes = [mp.Process(target = mpr,args = (mapset,di[x],x,abbreviation,coordDict,land,outputGates,outputGrid,moments,nfm)) for x in range(len(di))]
		#run processes
		for p in processes:
			p.start()
		...
		nfm.close()
		#end multiprocessing?
		#set the working mapset to the starting one:
		grass.run_command('g.mapset',mapset=mapset)
		ou=open("mapList.txt","w")
		ouG=open("mapListG.txt","w")
		nfR=open('nomiFmaps','r')
		nfmR=nfR.readlines()
		for i in nfmR[:-1]:
			mappa=i.split('@')[0]
			grass.run_command('g.copy',vect=i+','+mappa+'@'+str(mapset))		
		for i in range(1,len(di)+1):
#			grass.run_command('g.copy',vect=outputGrid+'_'+str(i)+'@tbr'+str(i-1)+','+outputGrid+'_'+str(i)+'@'+str(mapset))
#			grass.run_command('g.copy',vect=outputGates+'_'+str(i)+'@tbr'+str(i-1)+','+outputGates+'_'+str(i)+'@'+str(mapset))
			ou.writelines(outputGrid+'_'+str(i)+'|'+moments[i-1]+'|'+moments[i])
			ouG.writelines(outputGates+'_'+str(i)+'|'+moments[i-1]+'|'+moments[i])
			ou.writelines('\n')
			ouG.writelines('\n')
		ou.close()
		ouG.close()
		grass.run_command('t.register',input='traffic',type='vect',file='mapList.txt')
		grass.run_command('t.register',input='trafficG',type='vect',file='mapListG.txt')
		if animation:
#			mapset=grass.read_command('g.mapset',flags='p').split('\n')[0]
			print "load stvds trafficG too and customize your animation!"
			grass.run_command('g.gui.animation',stvds='traffic@'+mapset)
